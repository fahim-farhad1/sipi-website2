...
const AddDepartment = ({ refetch }) => {
  ...
  useEffect(() => {
    if (objectivesFields.length === 0) {
      addObjectives("");
    }
    if (featuresFields.length === 0) {
      addFeatures("");
    }
    if (successFields.length === 0) {
      addSuccess("");
    }
    if (requirementsFields.length === 0) {
      addRequirements("");
    }
    if (labFacilitiesFields.length === 0) {
      addLabFacilities("");
    }
    if (courseOutlineFields.length === 0) {
      addCourseOutline({ semester: "", subjects: [""] });
    }
    if (galleryImagesFields.length === 0) {
      addGalleryImages("");
    }
  }, [
    addObjectives,
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axiosPublic.post("/Department", structuredData);
